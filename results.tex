...
\textbf{Permission Incompatibility Issues (PRM).}
According to the experimental results, %shown in Table~\ref{tab:tab-results}, 
\@approach detects two cases of permission incompatibility issues in
...
higher than 23, yet do not follow the new runtime
permission checking. Note that the other mismatch
detection techniques do not detect any runtime
permission compatibility issues.  
%Next, we evaluate whether \@approach is sufficiently robust to handle large real-world apps.
...
Since manually examining all 3,571 real-world apps prohibitively expensive, we sampled 60 apps where incompatibilities were detected and calculated the precision scores via a manual analysis of the app code. 
We do not consider recall because the ground-truth incompatibilities are unknown. 
For those 60 apps, the precision scores for API invocation, callback, and permission incompatibilities are 85\%, 100\%, and 100\%, respectively. 
...
We then investigated \@approach's results to appraise its utility in practice. 
In the following, we report an illustrative subset of the detected incompatibilities, each of which we have reported to the app's
developers. 
%To avoid revealing previously
...
11, but the app's
\textsf{minSdkVersion} is API level 8. Therefore, if
...
\textbf{API callback mismatch.} FOSDEM~\cite{fosdem}, a
conference companion app, exhibits an API callback
mismatch in its \textsf{ForegroundLinearLayout} class, which
overrides the \textsf{View.drawableHotspotChanged} callback
method introduced in API level 21. The app's
\textsf{minSdkVersion} is API level 15, which would
not support the aforementioned callback, and in turn
