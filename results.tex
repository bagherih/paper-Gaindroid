...
\commentws{ By performing analysis incrementally, the
analysis cost is amortized over time.  This also
results in shorter start up time as only a small
portion of the code is needed to begin analysis.
\textsc{Semeo} can perform analysis in one pass whereas
\textsc{CID} needs two passes to detect compatibility
issues (see Section~\ref{limitations}). These factors
make our approach highly efficient.  Furthermore, the
amount of memory that must be committed to store code
and perform incremental analysis is also small,
resulting in fewer page faults and more available
memory to support analysis. This can enhance the
