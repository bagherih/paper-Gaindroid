...
crash the app.
\commentty{Need to clarify the overriding a callback will cause
incompatibility, raised by ICSE reviewer1.}
...
\textsc{Cider} \commentty{need citiation} conserves memory by creating models from the
...
\tym{Another state-of-the-art  incompatibility detector is \textsc{CiD}. 
\commentty{add citation}. To  conserve memory, \textsc{CiD} extracts
...
\tym{For example, if an application makes an API call, SAINTdroid identifies the class to which the invoked API belongs, 
