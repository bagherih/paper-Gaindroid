...
\label{limitations}
\commentws{
One major short-coming of the state-of-the-art
...
program analysis approaches operate like a compiler;
that is, it first loads all code in the project and
then performs analysis on the loaded code.  This
approach is monolithic and can be memory intensive.
For an existing state-of-the-art technique
to support the analysis of application and ADF in
unison, it would need to load the application and the
ADF code together as part of the analysis. Loading both
components is not feasible due to high memory 
consumption. As such, they analyze an app and the
underlying ADF separately, as shown in
Figure~\ref{fig:compare}a.

Existing techniques also face another challenge when 
analyzing the ADF code. For example, \textsc{Soot}, a
widely used Java program analysis framework, starts its
analysis from the \texttt{main} method in each program.
When \textsc{FlowDroid} adapts \textsc{Soot} to analyze
Android apps that can have multiple entry points,
\textsc{FlowDroid} developers created a workaround
utilizing dummy \texttt{main} methods. Such an approach
is not suitable for analyzing the ADF as it has no
\texttt{main} method, and an entry point can be any
supported API. As such, existing techniques model the
ADF code to perform analysis. 
}

For example, a state-of-the-art technique, called
\textsc{Cider}~\cite{huang2018understanding}, creates
models of the underlying ADF to represent API
invocations and their callback counterparts.  However,
constructing a complete ADF model can be laborious task
that may not be able to keep up with rapid releases of
ADFs~\cite{vanderMerwe2012}.  Thus, these analysis
techniques use abstract models that are inexpensive to
build, but they also lack finer details.  As will be
shown later, \textsc{Cider} can miss detecting
compatibility issues that exist in different ADFs.
...
In the next section, we describe how \@approach
overcomes these limitations to provide more capability
and accuracy in detecting API compatibility issues. 
%}

%Unlike \textsc{CiD}, our approach
...
%straightforward analysis with fewer analysis passes,
